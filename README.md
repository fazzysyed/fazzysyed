# 👋 Hi, I'm Faraz Syed

> ⚡️ React Native Architect | Full Stack Enthusiast  
> 🛠️ 6+ Years Experience | 🚀 Delivered 87+ Projects  
> 💻 Backend: Node.js | Laravel | AI/ML Integration

---

<p align="center" style="color:gray; font-style: italic;">
  💬 "SyntaxError: Unexpected token 'Faraz' at line 23" — Fixing with AI 🤖
</p>

<p align="center">
  <img
    src="https://github.com/fazzysyed/fazzysyed/blob/5679ae30fd3d492b082c5410ed7f70c722737eab/animation.gif?raw=true"
    alt="Chrome Dino Game - Black Theme"
    style="width: 100%; max-width: 1000px;"
  />
</p>


---

I specialize in building **high-performance cross-platform mobile apps** using **React Native**, and crafting robust **backend systems** with **Node.js** and **Laravel**. From ride-sharing apps and CRM platforms to **AI-driven solutions** like object detection and real-time poker hand analysis — I build it all.

In addition to mobile, I create desktop software using **Electron.js**, and develop APIs that are secure, scalable, and efficient.

---

## 🧠 Tech Stack & Tools

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,ts,js,nodejs,laravel,firebase,electron,tensorflow,mongodb,mysql,postgres,git,github,vscode" alt="Tech Stack Icons" />
</p>

---

## 🌟 Project Highlights

- 🚗 **Ride-Sharing Applications**  
- 📈 **CRM & Business Tools**  
- 🤖 **AI Projects** – YOLO, TensorFlow Lite, Real-Time Detection  
- ♠️ **Poker Apps** – Card Detection, Probability Engine  
- 📲 **Push Notification Systems (Firebase)**  
- 📞 **Video & Voice Calling** – WebRTC, Agora, Zoom SDK  
- 🖥️ **Electron-based Desktop Applications**  
- ⚙️ **Backend APIs** – Node.js, Laravel

---

## 🚀 Current Focus

- Develop mobile application using AI.
- Finding Peace

---

## 📬 Contact Me

📍 **Location:** Islamabad, Pakistan  
✉️ **Email:** fazzysyed75@gmail.com

---

## 🔗 Let's Connect

<p align="center">
  <a href="https://www.linkedin.com/in/farazsyedf16/" target="_blank" rel="noopener noreferrer" style="margin: 0 8px;">
    <img src="https://img.shields.io/badge/-LinkedIn-blue?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://github.com/fazzysyed" target="_blank" rel="noopener noreferrer" style="margin: 0 8px;">
    <img src="https://img.shields.io/badge/-GitHub-black?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
  </a>
<!--   <a href="https://www.fiverr.com/s/yvYb4r0" target="_blank" rel="noopener noreferrer" style="margin: 0 8px;">
    <img src="https://img.shields.io/badge/-Fiverr-1DBF73?style=flat-square&logo=fiverr&logoColor=white" alt="Fiverr" />
  </a>
  <a href="https://www.upwork.com/freelancers/~0142c7a9c7e72c5e7b?mp_source=share" target="_blank" rel="noopener noreferrer" style="margin: 0 8px;">
    <img src="https://img.shields.io/badge/-Upwork-6fda44?style=flat-square&logo=upwork&logoColor=white" alt="Upwork" />
  </a> -->
</p>

---

💡 *“You got money for war but can’t even feed the poor”* — *Tupac*
